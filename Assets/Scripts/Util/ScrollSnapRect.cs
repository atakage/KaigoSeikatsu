...
// RequireComponent: スクリプトをオブジェクトに追加すると自動でコンポーネントをつける(Image, Mask, ScrollRect)
[RequireComponent(typeof(Image))]
[RequireComponent(typeof(Mask))]
[RequireComponent(typeof(ScrollRect))]
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {

    // Tooltip: unityのinspector内に宣言された変数の説明
    ...
    void Update() {
        ...
        if (_lerp) {
            ...
            if (Vector2.SqrMagnitude(_container.anchoredPosition - _lerpTo) < 0.25f) { // _container.anchoredPositionと_lerpToの間が0.25f以下なら？
    ...
    private void SetPagePositions() {
        ...
        if (_horizontal) {
            ...
            offsetX = width / 2;
            // total width
            containerWidth = width * _pageCount; // containerのchild数くらいwidth設定
    ...
    private void SetPage(int aPageIndex) {
        aPageIndex = Mathf.Clamp(aPageIndex, 0, _pageCount - 1); // 入力したvalueがminとmax間ならそのままreturn, 入力したvalueがminより低いとminをreturn
    ...
    //------------------------------------------------------------------------
    // PointerEventData: マウスクリック時クリック回数、ボタン、ポインター位置の情報などを含めている
